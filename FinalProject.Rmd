...
#Based on Boxplot for Respiration Rate, many outliers are identified. NA anything outside the minimum and maximum 1.5IQR
#1.5IQR; if above 20.48 or below 15.085, remove from data set; This range is more realistic for an adult
##gh_tidy$bpdiastolic[gh_tidy$bpdiastolic < 15.085] <- NA
##gh_tidy$bpdiastolic[gh_tidy$bpdiastolic > 20.48] <- NA
...
gh_reg_icu1 = gh_reg %>% filter(icu_flag == 1)
gh_reg_icu0 = gh_reg %>% filter(icu_flag == 0)

#fit models
model_icu1 <- lm(gh_reg$log_losday~ ., data = gh_reg_icu1)
summary(model_icu1)
anova(model_icu1)

model_icu0 <- lm(gh_reg$log_losday~ ., data = gh_reg_icu0)
summary(model_icu0)
anova(model_ic0)
```
